...
"""
class Node:
    def __init__(self, max_reward=-np.inf, immediate_reward=np.inf, children=None, visits=0):
    ...
    while steps < max_episode_steps:
        if node is None:
            ...
            if random.random() < epsilon:
                ...
                best_actions = [
                    action for action, reward in action_reward.items() if reward == best_reward
                ]
...
def look_ahead(start_node, target_reward):
    #look ahead 8 steps (this will be 30 frames or ~1 second)
    # we only care about the eigth step, so get there 
    ...
    for _ in range(look_ahead_steps):
        ...
        for node in next_node.children.values():
            ...
            if node.max_reward == target_reward:
                #this could be the path we want to take
    ...
    return target_node.immediate_reward
...
class Grimm_Johan:
    ...
    def load_actions(self):
        ...
        self.node_count += update_path(self._root, executed_actions, rewards, total_reward, loaded=True)
        print(f'\x1B[34mInitial info: nodes: {self.node_count}, reward from saved input: {total_reward}!\x1B[0m')

    def run(self):
        actions = select_actions(self._root, self._env.action_space, self._max_episode_steps)
        steps, rewards, total_reward = perform_actions(self._env, actions)
        executed_actions = actions[:steps]
        self.node_count += update_path(self._root, executed_actions, rewards, total_reward)
...
def grimm_runner(
    ...
    johan = Grimm_Johan(env, max_episode_steps=max_episode_steps, actions=loaded_actions)
    timesteps = 0
    best_rew = float('-inf')
    while True:
        ...
        print('info: counter={}, timesteps={}, nodes={}, reward={}'.format(counter, timesteps, johan.node_count, rew))
